...
import TimeFactorForm from './TimeFactorForm';
import TimeFactorTable from './TimeFactorTable';

const API_ENDPOINT = '/time-factors';
const ENTITY_NAME = 'Factor de Tiempo';
const initialFormState = { id: '', description: '', value: 0.0, status: 'A' };

const TimeFactorPage = () => {
    ...
    <div className="p-6 bg-gray-100 min-h-screen">
      <div className="max-w-6xl mx-auto bg-white p-8 rounded-lg shadow-lg">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold text-gray-800">Factores de tiempo</h1>
          <Link to="/" className="px-4 py-2 text-sm font-medium text-white bg-gray-600 rounded-md hover:bg-gray-700">← Volver al Menú</Link>
        </div>

        <TimeFactorForm formData={formData} handleFormChange={handleFormChange} handleSubmit={handleSave} isLoading={isLoading} isEditing={isEditing} />
        ...
        <TimeFactorTable records={records} handleSelectRecord={setSelectedRecord} selectedRecordId={selectedRecord?.id} isLoading={isLoading} />
      </div>
    </div>
  );
};

export default TimeFactorPage;
