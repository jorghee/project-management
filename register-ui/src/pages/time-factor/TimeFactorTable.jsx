...
const TimeFactorTable = ({ records, handleSelectRecord, selectedRecordId, isLoading }) => {
  return (
      ...
      <table className="min-w-full divide-y divide-gray-200">
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          ...
          {!isLoading && records.map((record) => (
            <tr
              key={record.id} onClick={() => handleSelectRecord(record)}
              className={`cursor-pointer transition-colors ${record.status === '*' ? 'bg-red-50 text-gray-500' : ''} ${selectedRecordId === record.id ? 'bg-blue-200' : 'hover:bg-gray-50'}`}
            >
              ...
              <td className="px-6 py-4 whitespace-nowrap text-sm">
                {typeof record.value === 'number' ? record.value.toFixed(2) : 'N/A'}
              </td>
...
export default TimeFactorTable;
