...
import UtilityFactorForm from './UtilityFactorForm';
import UtilityFactorTable from './UtilityFactorTable';

const API_ENDPOINT = '/utility-factors';
const ENTITY_NAME = 'Factor de Utilidad';
const initialFormState = { id: '', description: '', value: 0.0, status: 'A' };

const UtilityFactorPage = () => {
    ...
    <div className="p-6 bg-gray-100 min-h-screen">
      <div className="max-w-6xl mx-auto bg-white p-8 rounded-lg shadow-lg">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold text-gray-800">Factores de Utilidad</h1>
          <Link to="/" className="px-4 py-2 text-sm font-medium text-white bg-gray-600 rounded-md hover:bg-gray-700">← Volver al Menú</Link>
        </div>

        <UtilityFactorForm formData={formData} handleFormChange={handleFormChange} handleSubmit={handleSave} isLoading={isLoading} isEditing={isEditing} />

        <div className="flex flex-wrap gap-3 mb-6">
          <button onClick={handleSave} disabled={isLoading} className="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700 disabled:bg-blue-400">{isEditing ? 'Guardar Cambios' : 'Crear Factor Utilidad'}</button>
        ...
        <UtilityFactorTable records={records} handleSelectRecord={setSelectedRecord} selectedRecordId={selectedRecord?.id} isLoading={isLoading} />
      </div>
    </div>
  );
};

export default UtilityFactorPage;
